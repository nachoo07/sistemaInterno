...
const Student = () => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      setWindowWidth(newWidth); // Actualizamos el estado del ancho de la ventana
      if (newWidth <= 576) {
        setIsMenuOpen(false); // Cerrar el menú en tamaño móvil
      } else {
        setIsMenuOpen(true); // Abrir el menú en tamaño escritorio
      }
    };

    // Ejecutar handleResize al montar el componente para establecer el estado inicial
    handleResize();
  ...
  const filteredStudents = estudiantes.filter((estudiante) => {
    ...
    const cuilSearch = estudiante.cuil?.toLowerCase().includes(searchNormalized);
  ...
  const handleDelete = async (studentId) => {
    try {
      ...
      Swal.fire("¡Éxito!", "El alumno ha sido eliminado.", "success");
    } catch (error) {
